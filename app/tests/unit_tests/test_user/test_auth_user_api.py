...
async def test_register_user(name,phone_number,del_address,email, password, status_code, ac: AsyncClient):
...
async def test_login_user(name,phone_number,del_address,email, password, status_code, ac: AsyncClient):
    response = await ac.post(
        "/auth/cookie/login",
        data={
            "name":name,
